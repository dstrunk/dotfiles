...
Enable =smartparens=, and fix some clashing that will occur with =smartparens= and =web-mode=.
...
Configure =smartparens= via https://github.com/Fuco1/smartprens/issues/286
...
Per http://emacs.stackexchange.com/questions/15188/smartparens-and-web-mode-conflict-to-add-extra-angular-bracket,
turn off auto-pairing for =web-mode= and enable paring for `<` and `%` in smartparens.
#+BEGIN_SRC emacs-lisp
(sp-pair "%" "%" :wrap "C-%")
(sp-pair "<" ">" :wrap "C->")
(sp-pair "?php" "?" :wrap "C-?")
...
Wrap at 80 characters, like every good program should.
...
#+BEGIN_SRC emacs-lisp
  (dir-locals-set-class-variables 'org-journal-settings
    '((nil . ((mode . org-journal)))))

  (defun dstrunk/org-journal-hook ()
  ...
  (add-hook 'org-journal-mode-hook 'dstrunk/org-journal-hook)
